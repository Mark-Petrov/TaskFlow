...
router.post('/', async (req, res) => {
  ...
  const boardId = routeParam(req, 'boardId')
  const {
    column_id, title, description, bg_color, border_color, border_style, badges,
  } = req.body
  ...
  await notifyBoardMembers(
    boardId, userId, task.id, 'task_created',
    `${actorName} добавил задачу «${task.title}» в «${board?.title}»`,
  )
...
router.patch('/:taskId', async (req, res) => {
  ...
  if (isCompleted !== undefined && existing.isCompleted !== isCompleted) {
    ...
    await notifyBoardMembers(
      boardId, userId, task.id, 'task_updated',
      `${actorName} изменил задачу на «${task.title}»`,
    )
  }
